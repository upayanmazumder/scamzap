    ...
    useEffect(() => {
        const fetchUsersAndFollowing = async () => {
            try {
                ...
                const resFollowing = await fetch(`${API}/users/following/${currentUserId}`);
        ...
        <main>
            ...
            <ul className="space-y-4">
                {users.map((user) => {
                    ...
                    return (
                        <li key={user._id} className="border p-4 rounded-md flex justify-between items-center transition">
                            ...
                            {!isCurrentUser && (
                                <button
                                    ...
                                    className={`ml-4 px-4 py-2 rounded-md font-semibold transition ${isFollowing
                                        ? "bg-red-500 text-white hover:bg-red-600"
                                        : "bg-blue-500 text-white hover:bg-blue-600"
                                        }`}
                                >
                                    {processing === user.id ? "Processing..." : isFollowing ? "Unfollow" : "Follow"}
                                </button>
                            )}
                        </li>
                    );
